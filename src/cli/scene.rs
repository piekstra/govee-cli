...
async fn handle_activate(
    ...
    if let Some(capabilities) = scenes.get("capabilities").and_then(|v| v.as_array()) {
                ...
                if let Some(options) = params.get("options").and_then(|v| v.as_array()) {
                    ...
                    if let Some(scene) = found {
                        let scene_name = scene
                            ...
                            .and_then(|n| n.as_str())
                            .unwrap_or(name);
                        if let Some(value) = scene.get("value") {
                            let param_id = value
                                .get("paramId")
                                .cloned()
                                .unwrap_or(json!(0));
...
fn extract_scene_names(data: &serde_json::Value) -> Vec<serde_json::Value> {
